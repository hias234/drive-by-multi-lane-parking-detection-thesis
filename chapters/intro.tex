...
There are several approaches to sense parking availability. A few of them will be discussed in chapter \ref{chap:relatedwork}. In this thesis the a "drive-by park sensing" approach will be implemented. Several sensing vehicles drive through the city and each of them is equipped with a few sensors, namely a distance sensor to measure the distance to the nearest obstacle on the right side of the road and a GPS receiver which determines the current position of the vehicle. 

 \cite{Mathur:2010:PDS:1814433.1814448} et al. 
