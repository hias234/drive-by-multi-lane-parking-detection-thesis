...
Sensing a city's parking availability and thus making the parking situation more transparent would be highly beneficial for reducing traffic congestion and greenhouse gas emissions as driver's looking for parking spaces in urban areas could navigate directly to vacant parking spaces close to their destinations. To support this goal, a prototype of drive-by sensing using an optical distance sensor has been designed, implemented, and evaluated. This chapter introduces the prototype and testbed which have been developed to acquire the necessary dataset to run the machine learning experiments. First, the used hardware and sensors are described (Section \ref{sec:test_bed}), then the software to collect test data is discussed (Section \ref{sec:sensor_measurement_collection}) and finally the acquired raw sensor dataset is presented (Section \ref{sec:raw_dataset}).
...
Figure \ref{fig:sensing_car} shows the sensing car and its mounted sensors. For collecting, processing, and saving the sensed data, a Raspberry Pi 2 Model B\footnote{\url{https://www.raspberrypi.org/products/raspberry-pi-2-model-b/}} is used. A Raspberry Pi is chosen because of its simplicity to connect and access sensors and moreover because it is easy to program as it is just a regular Linux-based computer. Furthermore, the price of a Raspberry Pi is also quite low (about \euro{35}), therefore the overall cost of the system will remain low.
...
To determine the location of the sensing vehicles while driving through the city, a Navilock USB GPS receiver\footnote{\url{http://www.navilock.de/produkte/G_61840/merkmale.html?setLanguage=en}} is used. The receiver is connected to the Raspberry Pi using a USB port. The sensor measures the GPS location (latitude and longitude) at a rate of about 1 Hz. According to the manufacturer, the acquired positions are correct in the range of 2.5 meters. The GPS receiver is mounted with a magnet on the top of the sensing car in order to receive more accurate sensor readings. The Navilock USB GPS sensor costs about \euro{50}.

Two major distance sensor technologies can be differentiated: an ultrasonic range sensor and an optical laser distance sensor. 
Experiments of drive-by parking space sensing using an ultrasonic range sensor have been performed prior to this thesis. 
However, while ultrasonic sensors are cheap and widely used in commercial parking assistance systems, they also have limitations in terms of sensing frequency (about 20 Hz) and range (a few meters). These specifications limit the use of ultrasonic sensors for drive-by sensing on multi-lane roads or high speed. Due to these limitations an optical distance sensor, the "Lidar Lite v3\footnote{\url{https://www.sparkfun.com/products/14032}}" is used in our testbed. It measures the time of flight of an emitted laser signal which is reflected by an object in its way. The Lidar Lite v3 can measure distances from a few centimeters up to 40 meters at a frequency of 1 to 500 Hz. Furthermore, it is able to measure distances with an accuracy of about 2.5 centimeters. However, in comparison to ultrasonic sensors, optical sensors are more expensive. While the costs of ultrasonic sensors are often below \euro{10}, the Lidar Lite v3 and other comparable sensors cost about \euro{150}.
...
\caption{Comparison of an ultrasonic sensor and the used Lidar Lite v3 optical distance sensor.}
...
Finally, the derived datasets are described in Section \ref{sec:derived_dataset}, providing an analysis of the datasets and the features. 
...
This fact makes it impossible to use the parking space map in the parking space sensing prototype as the parking spaces of the map cannot be matched with the sensed segments obtained during the experiments.
...
As parking space maps are unavailable in a sufficient quality from outside sources, an algorithm is introduced to derive a coarse parking space map from the dataset which was acquired during the test drives. The dataset contains the GPS positions of over 2.000 parking cars; most streets have been sensed several times. As of the limited amount of data, the goal is to create a coarse parking space map which contains areas where it is likely that vehicles park (parking zones). 

The proposed algorithm uses clustering to group parked cars to parking zones. The idea is that if there are several cars parking at the same location in several test drives then it is likely that there are parking spaces at that position. A density-based spatial clustering of applications with noise (DBSCAN\footnote{\url{http://scikit-learn.org/stable/modules/generated/sklearn.cluster.DBSCAN.html}} - part of the scikit learn\footnote{\url{http://scikit-learn.org/}} Python framework for machine learning) is used to cluster the positions of the parked cars. The DBSCAN algorithm searches for high density core samples and expands clusters from them. It takes the a similarity function which compares two data points and a minimum similarity as input parameters.

As similarity measure for the clustering algorithm a custom function has been implemented. Two parked cars are considered to be similar if both cars are close to each other (GPS position) and if they park on the same side of the street. The side of a parking car is determined by the direction the sensing vehicle is going while driving past the parking car. 
%The first check is applied to ensure that both compared cars are parking at the same side of the street. Else it would be possible that two cars which park at the opposite side of the street are considered as belonging to the same "parking zone". If the first check is passed, the similarity function will compute the distance in meters between the GPS positions of both parked cars which are compared. 
The described similarity function is used by the DBSCAN clustering algorithm to identify core samples and to cluster multiple parking cars to parking zones. The maximum distance between two cars to be considered to be similar is set to eight meters. If two parking cars are further apart they are not considered to be able to be in the same parking zone. 
The result of the algorithm are clusters of parking cars which are considered as "parking zones" or "area where it is likely that cars are parking". To be able to determine if a specific GPS position is in such a parking zone (needed later on for filtering), a bounding box is created for each cluster. Such a bounding box is created by taking the GPS position of the parking cars which are parking apart from each other the greatest distance. Around these two GPS positions a rectangle (the bounding box) is created by giving a ten meter error distance to all sides. The end result of the parking space map approximation is therefore a collection of parking zones, which are implemented as bounding boxes.
...
As argued before, it does not make sense to classify segments which are not close to parking zones. Such segments will not make a difference to the end result as it is known that at these positions parking is not possible (or legal) and there are not valid possible parking spaces. 
...
Two main datasets are available for the use in machine learning experiments. The "full dataset" contains all segments which have been sensed during the test drives while the "filtered dataset" has been filtered to only contain segments which are close to areas where parking spaces are located (described in the previous section). Both datasets are made up of samples with all calculated feature values (discussed in Section \ref{sec:feature_calculation}) and the corresponding class label.

Table \ref{table:dataset_comparison} shows a comparison of the number of instances per class for both datasets as well as the percentage of each class. One can clearly see that both datasets are highly imbalanced because in both cases the "free space" class is by far the dominating class with 83.6\% and 73.8\%, respectively. However, the filtered dataset contains only about half of the "free space" instances which are in the full dataset and thus the filtered dataset is expected to lead to a better classification result as it is more balanced. Of course, this has to be proven by the machine learning experiments which will be covered in the following sections.
...
Machine learning is used to classify a segment as one of the available parking situation (label).
To conduct machine learning experiments with common machine learning models features serve as input parameters. The feature values for each segment are computed using the sensor measurements. All computed features are described below. A further analysis of the usefulness of the features for the classification tasks will be provided in the next section.
...
\item[Duration:] The duration in seconds the sensing vehicle needed to sense the segment (the difference between last time stamp and the first time stamp).
...
\item[Distance difference to the next segment:] Difference of the average distance measurements to the next segment in meters.

\item[Distance difference to the previous segment:] Difference of the average distance measurements to the previous segment in meters.
...
ParkNet also uses the drive-by parking space sensing approach which is used in our prototype. 
...
To verify that length and distance only are not enough features, classification of segments with only these two features has been carried out on our dataset. A decision tree with a maximum depth of two has been learned using our dataset and evaluated using 10-fold cross-validation. Similar to the two thresholds used in ParkNet, our decision tree also learned to divide the two target classes using two thresholds. 
...
\label{chap:machine_learning_experiments}
...
The output of the neural network is calculated by a process called forward propagation. The resulting value of each unit is obtained by a non-linear formula using all the previous units' values. The output of a hidden layer is calculated using the following formula:

\[ f(x) = a(b + \sum_{i=0}^{n} x_i \times w_{i}) \]
 
The formula computes a sum of all input values $x_i$ multiplied by their weights $w_i$ and adds a constant bias value $b$. Subsequently, this intermediate result is applied to a non-linear activation function $a$. The same formula used to compute the result of the hidden units is also used for the output unit to calculate the final result of the neural network. The forward propagation of the neural network represents a function $f(x)$ which maps the input features $x$ to the output value $f(x)$.

To use neural networks for classification, the function $f(x)$ has to be learned according to provided training samples. As $f(x)$ is calculated using all weights $w_x$ and bias values $b_x$, these values have to be learned. This is done using back propagation. As first step all weights and biases are initialized randomly. Then, for every training sample the result of the neural network is computed. An error to the desired output is calculated using a loss function, such as the squared-error loss function. Using gradient decent, the weights and biases are then adapted slightly to reduce the loss function. This process is repeated and finally the loss function should be minimized to obtain good classification results. The most important parameters which can be set are the number of hidden layers and hidden units, the learning rate, and the activation function. Further information about neural networks and their parameters can be found in \cite{Hall2016_DataMining_ML}.
...
\begin{figure}
	...
	\caption{Example of a decision tree with a depth of two, which has been trained with our full parking space dataset using only the features length and distance.}
...
\lstinputlisting[caption={Example of a densely-connected deep learning network containing five hidden layers with dropout defined with Keras}, language=Python, label={lst:deep_learning1}, basicstyle=\small]{code/tensorflow_dense_neural_network.py}
...
To investigate if such information can in fact benefit the classification results, a custom two-staged classification model is implemented. First of all, a basic classification task is performed by the best single classifier, which is identified by prior experiments (see Section \ref{sec:machine_learning_models}). Then, an enhanced dataset is created by adding the class which was predicted by the first classifier to the existing dataset as well as by inserting features derived by the surrounding segments. For example, the average distance of the surrounding parking cars is added to the enhanced dataset. This should help the stage-two classifier to detect parking cars, because usually a parking car is parked at the same distance as surrounding parking cars. Furthermore, the average distances of the surrounding segments of all other classes are added as well.
 The number of surrounding segments used is tested in several experiments to find the amount, which performs at the highest accuracy. In the final step, another classification task is performed using the enhanced dataset. This process should help to include information of surrounding segments and it should improve the classification results. Figure \ref{fig:surrounding_segments_classification_process} shows the two-staged classification process and an example of an enhanced dataset which is used by the stage two classifier.
The results of the approach can be viewed in Section \ref{sec:using_surroundings_to_improve_results}.


\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{img/surrounding-segment-classification-process.eps}
	\caption{Two-staged classification process. After a stage 1 classifier initially predicts the class of the samples, its results are used to build an enhanced dataset including distance information of the segment's surroundings. A stage 2 classifier predicts the final result using this enhanced dataset.}
	\label{fig:surrounding_segments_classification_process}
\end{figure}
