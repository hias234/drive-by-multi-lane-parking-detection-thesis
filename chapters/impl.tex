...
Deep learning gained a lot of attention in the last few years because of its rising classification accuracy results. Deep learning models became the first models which created systems which could compete with humans on classification tasks. A deep learning network is a special form of a neural network containing a possibly large amount of hidden layers between the input layer and the output layer. Furthermore, in most applications as input raw sensor measurements are used instead of pre-computed feature values (which were derived from the raw sensor values). In theory, a neural network (and thus a deep learning network) can derive its own feature values from the raw sensor data given enough hidden elements and long enough training time.

In this thesis deep learning should be evaluated for the use of classifying parking situations using distance sensor data. It should be investigated if deep learning can be used to do this kind of classification on raw sensor data and should be compared with classical machine learning algorithms (described in Section \ref{sec:machine_learning_models}) using pre-computed feature values.
...
As input for the deep learning experiments a dataset containing all sensor measurements for each segment is needed. In a pre-processing step a dataset is created which is made of a vector of sensor measurements for each segment. Each vector has a fixed size of 1024 sensor measurements. In most cases the segments have a much lower number of measurements. In these cases the measurements will be zero padded to the required size. If a segment has more than 1024 measurements, only the first 1024 will be used. This step is necessary to have a fixed length input for the experiments. A vector represents the input layer of the neural network. 
...
As baseline deep learning network a dense neural network with 5 hidden layers and 64 hidden elements per layer is used. This basic deep learning network should show how a densely-connected DL model performs on the given dataset. Figure \ref{fig:densely_dl_network} shows the illustration of the created deep learning network containing of an input layer with 1024 elements (one for each sensor value), 5 hidden layers containing 64 hidden elements each and four output elements which calculate the probability of each class for the given input. 

%There already exist several ways to improve the performance of deep learning methods, such as dropout nodes, convolutional nodes and max pooling nodes. All of these techniques should be tested, so that several deep learning networks can be compared on their performance on the dataset.

\begin{figure}
	\centering
	\includegraphics[width=\textwidth]{img/deep_learning_basic_model.eps}
