...
Deep learning gained a lot of attention in the last few years because of its rising classification accuracy results. A deep learning network is a special form of a neural network containing a possibly large amount of hidden layers between the input layer and the output layer. Furthermore, in most applications as input raw sensor measurements are used instead of pre-computed feature values (which were derived from the raw sensor values). In theory, a neural network (and thus a deep learning network) can derive its own feature values from the raw sensor data given enough hidden elements and long enough training time.
