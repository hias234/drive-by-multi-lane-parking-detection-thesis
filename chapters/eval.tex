...
When comparing the different sampling techniques, under-sampling clearly performs worst. This can be explained because the under-sampling algorithm deletes most samples of the majority classes in the dataset to create a balanced new dataset. The under-sampled dataset has a size of only 240 samples in total whereas the filtered dataset has a size of 8989 samples. However, this process removes so much information when deleting these samples that the overall performance decreases drastically. Over-sampling and the combination technique show similar results in terms of accuracy.

The primary goal of this thesis is to find a classifier with high accuracy on the \emph{parking car}-class (see Section \ref{sec:optimization_goal}). Only the secondary goal is to improve the accuracy of the minority classes, which is the goal of the sampling process. As all sampling techniques decrease the performance on the \emph{parking car}-class, performing over- or under-sampling is counterproductive to the primary goal and therefore is not considered to help in the creation of accurate parking space availability estimates.
