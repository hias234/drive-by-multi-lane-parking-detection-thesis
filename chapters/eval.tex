...
Another conclusion which can be drawn is that the performance is much worse for the classes \emph{overtaking situation} and \emph{other parking vehicle} than for the \emph{free space}- or \emph{parking car}-class. The best random forest classifier reaches a recall of 0.2575 and a precision of 0.8947 on \emph{overtaking situations}. Therefore, only about a quarter of the overtaking situations are detected. The \emph{other parking vehicle}-class has even worse results: a recall of 0.0833 and a precision of 0.8333.
The bad results on the two classes are probably cased because of the really low number of segments of both classes in the datasets. Thus, the variety of the data is low and a lot of different cases are not included in the segments which have been sensed. Another explanation is again the the imbalance in both datasets.
...
As the datasets are highly imbalanced, sampling techniques have been tested on their effect to improve the accuracy of the classifier and to improve the accuracy for classes with a low number of samples. Table \ref{table:comparison_sampling_techniques} shows a comparison of different sampling techniques and their impact on different performance measures. The experiments have been performed with the best performing classifier of the previous section (random forest classifier) with the filtered dataset.

All sampling techniques alter the training data to be more balanced, before starting the training process. The test data is not altered to get comparable results to the experiments where no sampling has been performed.
