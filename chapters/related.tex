...
Bock et al. \cite{Bock2015} used two 2D LiDAR distance scanners to approach the problem of drive-by park sensing. They were both deployed on a regular car facing to the right side of the road with an angle of 60 degrees in between them. The position of the vehicle is measured via a GNSS sensor (global navigation satellite system) which is more accurate than standard GPS receivers (it reaches an accuracy of 15-30 cm in height and 20 cm in horizontal directions for urban scenarios).
